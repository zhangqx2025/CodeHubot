...
class QwenEmbeddingService(EmbeddingService):
    ...
    def __init__(self, api_key: str, base_url: str = "https://dashscope.aliyuncs.com/api/v1/services/embeddings/text-embedding/text-embedding"):
        super().__init__()
                ...
                async with httpx.AsyncClient(timeout=60.0) as client:
                    response = await client.post(
                        ...
                        json={
                            "model": self.model_name,
                            "input": {
                                "texts": [t[:2048] for t in batch]  # 限制长度
                            }
                        }
                    )
...
async def embed_document(
    ...
    try:
        ...
        logger.info(f"文档 {doc.id} 切分为 {total_chunks} 个文本块")
        ...
        if not failed_indices:
            ...
            logger.info(f"文档 {doc.id} 向量化完成，共 {len(chunk_objects)} 个文本块")
        else:
            doc.embedding_status = 'failed'
            doc.chunk_count = len(chunk_objects)
            doc.embedding_error = f"部分文本块向量化失败: {len(failed_indices)} 个（{failed_indices[:10]}...）"
            logger.error(f"文档 {doc.id} 部分向量化失败，成功 {len(chunk_objects)}/{total_chunks}")
