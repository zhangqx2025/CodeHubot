...
# 文件存储根目录
UPLOAD_DIR = Path("data/knowledge-bases")
...
async def preview_document_chunks(
    ...
    if file_size > MAX_FILE_SIZE:
        return error_response(message=f"文件大小超过限制（最大5MB）", code=400)
    ...
    try:
        ...
        return error_response(message=f"文档切分失败: {str(e)}", code=400)
...
async def upload_document(
    ...
    split_mode: str = "fixed",  # 切分方式: fixed/paragraph/sentence/custom
    chunk_size: Optional[int] = None,  # 自定义块大小
    chunk_overlap: Optional[int] = None,  # 自定义重叠大小
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """上传文档到知识库"""
    ...
    doc = Document(
        ...
        embedding_status='pending' if auto_embedding else 'completed',
