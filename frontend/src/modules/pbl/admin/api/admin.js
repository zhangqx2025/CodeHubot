...
export const getCourseTemplateResources = async (courseUuid) => {
  try {
    const response = await request.get(`admin/courses/${courseUuid}/template-resources`)
    return response.data.data || response.data
  } catch (error) {
    throw new Error(handleApiError(error))
  }
...
export const getCourseTemplateTasks = async (courseUuid) => {
  try {
    const response = await request.get(`admin/courses/${courseUuid}/template-tasks`)
    return response.data.data || response.data
  } catch (error) {
    throw new Error(handleApiError(error))
  }
...
export const copyResourceFromTemplate = async (courseUuid, unitUuid, resourceTemplateUuid) => {
  try {
    const response = await request.post(
      `/admin/courses/${courseUuid}/units/${unitUuid}/resources/copy-from-template`,
      null,
...
export const copyTaskFromTemplate = async (courseUuid, unitUuid, taskTemplateUuid) => {
  try {
    const response = await request.post(
      `/admin/courses/${courseUuid}/units/${unitUuid}/tasks/copy-from-template`,
      null,
