<template>
  <div class="admin-course-detail">
                    ...
                    <div class="section-header">
                      ...
                      <div class="header-actions">
                        <el-button 
                          type="success" 
                          size="small"
                          @click="showTemplateResourceDialog(unit)"
                        >
                    ...
                    <div class="section-header">
                      ...
                      <div class="header-actions">
                        <el-button 
                          type="success" 
                          size="small"
                          @click="showTemplateTaskDialog(unit)"
                        >
                          <el-icon><Document /></el-icon>
                          从模板选择
                        </el-button>
    ...
    <!-- 从模板选择资源对话框 -->
    <el-dialog
      v-model="templateResourceDialogVisible"
      title="从模板选择资源"
      width="800px"
    >
      <div v-loading="templateResourcesLoading">
        <el-alert 
          v-if="!templateResourcesLoading && templateResources.length === 0"
          title="模板中暂无资源"
          type="info"
          :closable="false"
          show-icon
        ...
        <el-table
          ...
          @current-change="(row) => selectedTemplateResource = row?.uuid"
          max-height="400px"
        >
      ...
      <template #footer>
        <el-button @click="templateResourceDialogVisible = false">取消</el-button>
        <el-button 
          type="primary" 
          @click="copyTemplateResource"
          :disabled="!selectedTemplateResource"
        >
          复制到当前单元
        </el-button>
      </template>
    ...
    <!-- 从模板选择任务对话框 -->
    <el-dialog
      v-model="templateTaskDialogVisible"
      title="从模板选择任务"
      width="900px"
    >
      <div v-loading="templateTasksLoading">
        <el-alert 
          v-if="!templateTasksLoading && templateTasks.length === 0"
          title="模板中暂无任务"
          type="info"
          :closable="false"
          show-icon
        ...
        <el-table
          ...
          :data="templateTasks"
          highlight-current-row
          @current-change="(row) => selectedTemplateTask = row?.uuid"
          max-height="400px"
        >
          <el-table-column type="index" label="序号" width="60" align="center" />
      ...
      <template #footer>
        <el-button @click="templateTaskDialogVisible = false">取消</el-button>
        <el-button 
          type="primary" 
          @click="copyTemplateTask"
          :disabled="!selectedTemplateTask"
        >
          复制到当前单元
        </el-button>
      </template>
...
const templateResourceDialogVisible = ref(false)
const templateTaskDialogVisible = ref(false)
...
// 模板资源和任务数据
const templateResources = ref([])
const templateTasks = ref([])
const selectedTemplateResource = ref(null)
const selectedTemplateTask = ref(null)
const currentUnit = ref(null)
const templateResourcesLoading = ref(false)
const templateTasksLoading = ref(false)
...
// 从模板选择资源
const showTemplateResourceDialog = async (unit) => {
  currentUnit.value = unit
  templateResourceDialogVisible.value = true
  templateResourcesLoading.value = true
  
  try {
    const data = await getCourseTemplateResources(courseId.value)
    templateResources.value = data.resources || []
  } catch (error) {
    if (error.message.includes('未关联课程模板')) {
      ElMessage.warning('该课程未关联课程模板，无法从模板选择资源')
      templateResourceDialogVisible.value = false
    } else {
      ElMessage.error('加载模板资源失败：' + error.message)
    }
  } finally {
    templateResourcesLoading.value = false
  }
}

// 从模板复制资源
const copyTemplateResource = async () => {
  if (!selectedTemplateResource.value) {
    ElMessage.warning('请选择要复制的资源')
    return
  }
  
  try {
    await copyResourceFromTemplate(
      courseId.value,
      currentUnit.value.uuid,
      selectedTemplateResource.value
    )
    ElMessage.success('资源复制成功')
    templateResourceDialogVisible.value = false
    selectedTemplateResource.value = null
    loadCourseDetail()
  } catch (error) {
...
// 从模板选择任务
const showTemplateTaskDialog = async (unit) => {
  currentUnit.value = unit
  templateTaskDialogVisible.value = true
  templateTasksLoading.value = true
  
  try {
    const data = await getCourseTemplateTasks(courseId.value)
    templateTasks.value = data.tasks || []
  } catch (error) {
    if (error.message.includes('未关联课程模板')) {
      ElMessage.warning('该课程未关联课程模板，无法从模板选择任务')
      templateTaskDialogVisible.value = false
    } else {
      ElMessage.error('加载模板任务失败：' + error.message)
    }
  } finally {
...
// 从模板复制任务
const copyTemplateTask = async () => {
  if (!selectedTemplateTask.value) {
    ElMessage.warning('请选择要复制的任务')
    return
  }
  
  try {
    await copyTaskFromTemplate(
      courseId.value,
      currentUnit.value.uuid,
    ...
    templateTaskDialogVisible.value = false
    selectedTemplateTask.value = null
    loadCourseDetail()
  } catch (error) {
    ElMessage.error('复制失败：' + error.message)
  }
