# 文件上传目录持久化配置说明

## 📁 上传目录说明

后端服务有以下几个需要持久化的目录：

### 1. `/app/uploads` - 资源文件上传目录
- **用途**: 存储 PBL 模块中上传的课程资源文件（视频、文档等）
- **相关代码**: `backend/app/api/pbl/admin_resources.py`
- **文件类型**: 
  - 视频: `.mp4`, `.avi`, `.mov`, `.wmv`, `.flv`
  - 文档: `.pdf`, `.doc`, `.docx`, `.txt`, `.md`, `.ppt`, `.pptx`

### 2. `/app/static/firmware` - 固件文件上传目录
- **用途**: 存储 ESP32 设备的固件 `.bin` 文件
- **相关代码**: `backend/app/api/firmware.py`
- **文件类型**: `.bin` (固件文件)

### 3. `/app/backend/data/knowledge-bases` - 知识库文档目录
- **用途**: 存储 AI 知识库的文档文件
- **相关代码**: 知识库相关 API
- **文件类型**: 各类文档文件

---

## 🐳 Docker 持久化配置

### Docker Volume 定义

在所有 Docker Compose 配置文件中，已添加以下 Volume 定义：

```yaml
volumes:
  uploads_data:          # 上传文件存储卷
  static_data:           # 静态文件存储卷（固件、资源等）
  knowledge_bases_data:  # 知识库文档共享存储卷
```

### 后端服务 Volume 挂载

```yaml
services:
  backend:
    volumes:
      # 知识库文档持久化
      - knowledge_bases_data:/app/backend/data/knowledge-bases
      # 资源文件上传目录持久化
      - uploads_data:/app/uploads
      # 静态文件持久化（固件等）
      - static_data:/app/static
```

---

## 📋 配置文件清单

以下配置文件已全部更新，添加了文件上传目录的持久化配置：

1. ✅ `docker/docker-compose.yml` - 基础开发环境
2. ✅ `docker/docker-compose.prod.yml` - 生产环境（完整服务）
3. ✅ `docker/docker-compose.external-db.yml` - 使用外部数据库
4. ✅ `docker/docker-compose.with-mqtt.yml` - 带 MQTT 服务的独立配置

---

## 🔧 Dockerfile 改进

### 创建必要目录

在 `backend/Dockerfile` 中添加了目录创建步骤：

```dockerfile
# 创建必要的上传目录
RUN mkdir -p /app/uploads /app/static/firmware /app/backend/data/knowledge-bases && \
    chmod -R 755 /app/uploads /app/static /app/backend/data
```

这确保了：
- 容器启动时目录已存在
- 目录具有正确的权限（755）
- 应用可以正常写入文件

---

## 🚀 部署说明

### 首次部署

```bash
# 1. 启动服务
cd docker
docker-compose -f docker-compose.prod.yml up -d

# 2. 查看 Volume 是否创建成功
docker volume ls | grep codehubot

# 预期输出：
# docker_uploads_data
# docker_static_data
# docker_knowledge_bases_data
```

### 数据迁移

如果已有旧容器运行中，需要迁移数据：

```bash
# 1. 停止旧容器
docker-compose down

# 2. 从旧容器复制数据到 Volume（如果需要）
docker run --rm -v old_container:/source -v docker_uploads_data:/target alpine sh -c "cp -r /source/* /target/"

# 3. 启动新容器
docker-compose up -d
```

### 数据备份

```bash
# 备份上传文件
docker run --rm -v docker_uploads_data:/data -v $(pwd):/backup alpine tar czf /backup/uploads-backup.tar.gz /data

# 备份静态文件（固件）
docker run --rm -v docker_static_data:/data -v $(pwd):/backup alpine tar czf /backup/static-backup.tar.gz /data

# 备份知识库文档
docker run --rm -v docker_knowledge_bases_data:/data -v $(pwd):/backup alpine tar czf /backup/kb-backup.tar.gz /data
```

### 数据恢复

```bash
# 恢复上传文件
docker run --rm -v docker_uploads_data:/data -v $(pwd):/backup alpine sh -c "cd /data && tar xzf /backup/uploads-backup.tar.gz --strip 1"

# 恢复静态文件
docker run --rm -v docker_static_data:/data -v $(pwd):/backup alpine sh -c "cd /data && tar xzf /backup/static-backup.tar.gz --strip 1"

# 恢复知识库文档
docker run --rm -v docker_knowledge_bases_data:/data -v $(pwd):/backup alpine sh -c "cd /data && tar xzf /backup/kb-backup.tar.gz --strip 1"
```

---

## 🔍 故障排查

### 检查 Volume 是否正确挂载

```bash
# 进入容器查看
docker exec -it codehubot-backend bash

# 检查目录是否存在
ls -la /app/uploads
ls -la /app/static/firmware
ls -la /app/backend/data/knowledge-bases

# 检查权限
ls -ld /app/uploads /app/static /app/backend/data
```

### 检查文件是否持久化

```bash
# 1. 上传一个测试文件（通过 API）

# 2. 重启容器
docker restart codehubot-backend

# 3. 检查文件是否还在
docker exec -it codehubot-backend ls -la /app/uploads
docker exec -it codehubot-backend ls -la /app/static/firmware
```

### 权限问题

如果出现权限错误，检查：

```bash
# 查看容器内进程的用户
docker exec -it codehubot-backend ps aux

# 修复权限（如果需要）
docker exec -it codehubot-backend chmod -R 755 /app/uploads /app/static /app/backend/data
```

---

## ⚠️ 注意事项

1. **Volume 命名**: Docker Compose 会自动添加项目名前缀，例如 `docker_uploads_data`
2. **数据安全**: 定期备份 Volume 数据，避免数据丢失
3. **磁盘空间**: 监控上传文件的磁盘使用情况，及时清理无用文件
4. **权限管理**: 确保容器内的应用有足够的权限写入这些目录
5. **环境变量**: 如果修改了目录路径，需要在环境变量中同步更新

---

## 📊 磁盘使用监控

```bash
# 查看所有 Volume 的磁盘使用情况
docker system df -v

# 查看特定 Volume 的大小
docker run --rm -v docker_uploads_data:/data alpine du -sh /data
docker run --rm -v docker_static_data:/data alpine du -sh /data
docker run --rm -v docker_knowledge_bases_data:/data alpine du -sh /data
```

---

## 📝 相关代码位置

- 固件上传: `backend/app/api/firmware.py:160-162`
- 资源上传: `backend/app/api/pbl/admin_resources.py:28-29`
- 静态文件挂载: `backend/main.py:199-201`
- Dockerfile: `backend/Dockerfile:39-41`

---

**最后更新时间**: 2025-12-20


