...
本文档说明如何在远程服务器上更新已部署的 CodeHubot 系统。

---

## 📋 更新内容概述

本次更新添加了文件上传目录的 Docker Volume 持久化配置，主要改动：

- ✅ 添加 `uploads_data`、`static_data` Volume 持久化
- ✅ 后端 Dockerfile 自动创建上传目录
...
# 备份上传文件（如果有的话）
docker cp codehubot-backend:/app/uploads ./uploads-backup 2>/dev/null || echo "没有 uploads 目录"
docker cp codehubot-backend:/app/static ./static-backup 2>/dev/null || echo "没有 static 目录"
...
如果需要从旧的文件存储方式迁移到新的 Volume 方式，使用此方法。
...
```bash
# 备份数据库
docker exec codehubot-mysql mysqldump -uaiot_user -p[密码] aiot_admin > ~/codehubot-backups/db-backup-$(date +%Y%m%d_%H%M%S).sql
...
# 恢复数据库（如果需要）
# docker exec -i codehubot-mysql mysql -uaiot_user -p[密码] aiot_admin < ~/codehubot-backups/db-backup-xxx.sql
...
# 如果名称前缀不同，需要调整命令中的 Volume 名称
# 例如：codehubot_uploads_data 而不是 docker_uploads_data
```
...
# 如果旧容器还在，可以从中复制文件
docker cp <旧容器ID>:/app/uploads ./recovered-uploads
